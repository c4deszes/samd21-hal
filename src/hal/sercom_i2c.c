...
#define SERCOM_I2C_CMD_REPEAT_START 1U
#define SERCOM_I2C_CMD_READ 2U
#define SERCOM_I2C_CMD_STOP 3U
...
static sercom_i2c_result_t i2c_wait_read_ready(sercom_registers_t* peripheral) {
    uint32_t timeout = SERCOM_I2C_TIMEOUT_LOOPS;
    while ((peripheral->I2CM.SERCOM_INTFLAG & (SERCOM_I2CM_INTFLAG_SB_Msk | SERCOM_I2CM_INTFLAG_MB_Msk)) == 0U) {
        if ((peripheral->I2CM.SERCOM_INTFLAG & SERCOM_I2CM_INTFLAG_ERROR_Msk) != 0U) {
            return i2c_status_to_result(peripheral);
        }
        if (--timeout == 0U) {
    ...
    if ((peripheral->I2CM.SERCOM_INTFLAG & SERCOM_I2CM_INTFLAG_MB_Msk) != 0U) {
        return i2c_status_to_result(peripheral);
    }
...
static sercom_i2c_result_t i2c_send_stop(sercom_registers_t* peripheral) {
    i2c_write_ctrlb(peripheral, SERCOM_I2CM_CTRLB_CMD(SERCOM_I2C_CMD_STOP));
...
void SERCOM_I2C_SetupMaster(uint8_t sercom, uint32_t clock_in, uint32_t baudrate) {
    ...
    peripheral->I2CM.SERCOM_CTRLB = 0U;
...
sercom_i2c_result_t SERCOM_I2C_Write(uint8_t sercom, uint8_t address,
    ...
    return SERCOM_I2C_WriteEx(sercom, address, data, size, true);
}
...
sercom_i2c_result_t SERCOM_I2C_WriteEx(uint8_t sercom, uint8_t address,
    ...
    return i2c_write_internal(peripheral, address, data, size, stop);
}

sercom_i2c_result_t SERCOM_I2C_Read(uint8_t sercom, uint8_t address,
                                    uint8_t* data, uint16_t size) {
    return SERCOM_I2C_ReadEx(sercom, address, data, size, true);
}
...
sercom_i2c_result_t SERCOM_I2C_ReadEx(uint8_t sercom, uint8_t address,
    ...
    /* Reset ACK action and command between transactions. */
    i2c_write_ctrlb(peripheral, 0U);

    peripheral->I2CM.SERCOM_ADDR = SERCOM_I2CM_ADDR_ADDR(((uint32_t)address << 1U) | 1U);

    result = i2c_wait_read_ready(peripheral);
    if (result != sercom_i2c_ok) {
        (void)i2c_send_stop(peripheral);
        return result;
    }

    data[0] = peripheral->I2CM.SERCOM_DATA;

    for (uint16_t i = 1; i < size; i++) {
        i2c_write_ctrlb(peripheral, SERCOM_I2CM_CTRLB_CMD(SERCOM_I2C_CMD_READ));
        result = i2c_wait_sync(peripheral, SERCOM_I2CM_SYNCBUSY_SYSOP_Msk);
        if (result != sercom_i2c_ok) {
            (void)i2c_send_stop(peripheral);
            return result;
        }
    ...
    i2c_write_ctrlb(peripheral, SERCOM_I2CM_CTRLB_ACKACT_Msk |
                                SERCOM_I2CM_CTRLB_CMD(stop ? SERCOM_I2C_CMD_STOP : SERCOM_I2C_CMD_REPEAT_START));
    return i2c_wait_sync(peripheral, SERCOM_I2CM_SYNCBUSY_SYSOP_Msk);
}
...
sercom_i2c_result_t SERCOM_I2C_WriteRead(uint8_t sercom, uint8_t address,
    ...
    if (tx_size == 0U) {
        return SERCOM_I2C_ReadEx(sercom, address, rx_data, rx_size, true);
    }
